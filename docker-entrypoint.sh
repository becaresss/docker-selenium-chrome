...
if [ -n "$USER" ]; then
	...
	chown ${USER}:${USER} /tmp/selenium-server-standalone-3.0.1.jar
	chown ${USER}:${USER} /tmp/chromedriver

	su - ${USER} -c "mkdir -p /home/${USER}/log/selenium"
	su - ${USER} -c "java -Dwebdriver.chrome.driver=/tmp/chromedriver -jar /tmp/selenium-server-standalone-3.0.1.jar -host $HOSTNAME -role node -hub http://${SELENIUM_GRID:=seleniumgrid.cd.stratio.com:4444}/grid/register -browser browserName=chrome,version=56${ID} >> /home/${USER}/log/selenium/selenium-chrome.log &"
